...
export function RepoSelection({ onSelect }: Readonly<RepoSelectionProps>) {
    ...
    const handlePublicSubmit = () => {
        ...
        try {
            ...
            if (pathParts.length > 0) {
                name = pathParts.at(-1)?.replace('.git', '') || "public-project";
            }
        } catch { /* fallback */ }
    ...
    if (loading) {
        content = <div className="p-8 text-center text-slate-500">Loading GitHub connections...</div>;
    } else if (tab === "github") {
        if (isConnected) {
            content = (
                <>
        ...
        content = (
            <div className="max-w-xl mx-auto space-y-4 py-8">
                ...
                <div className="space-y-2">
                    <label htmlFor="repo-url" className="text-sm font-bold text-slate-900 dark:text-white ml-1">Git Repository URL</label>
