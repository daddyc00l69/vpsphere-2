...
type DeploymentState = "progress" | "success" | "failure";
...
const STEPS = [
    { id: 1, label: "Queued", duration: 1000 },
    { id: 2, label: "Provisioning", duration: 2000 },
    { id: 3, label: "Building", duration: 4000 },
    { id: 4, label: "Deploying", duration: 3000 },
    { id: 5, label: "Health Check", duration: 2000 },
];
...
export function DeploymentStatus({ startState = "progress", serviceId }: DeploymentStatusProps) {
    ...
    useEffect(() => {
        ...
        const processSteps = async () => {
            for (const step of STEPS) {
                ...
                setLogs(prev => [...prev, `[INFO] Starting ${step.label}...`]);
                await new Promise(resolve => setTimeout(resolve, step.duration / 2));
                setLogs(prev => [...prev, `[INFO] Completed ${step.label}.`]);
                await new Promise(resolve => setTimeout(resolve, step.duration / 2));
            }
            if (Math.random() > 0.1) {
                setState("success");
                ...
                setState("failure");
                setLogs(prev => [...prev, "[ERROR] Health check failed. Container exited with code 1."]);
            }
        };
                ...
                <div className="flex justify-between items-center mb-6">
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">
                            {state === "progress" && "Deploying Service..."}
                            {state === "success" && "Deployment Successful!"}
                            {state === "failure" && "Deployment Failed"}
                        </h1>
                        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
                            ID: {serviceId} • Started just now
                        </p>
                    </div>
                    {state === "success" && (
                        ...
                        {state === "progress" && (
                            ...
                            >
                                <div className="flex items-center gap-4 mb-4">
                                    ...
                                    <div>
                                        <h3 className="font-bold text-slate-900 dark:text-white">Build in Progress</h3>
                                        <p className="text-xs text-slate-500">Estimated time: ~45s</p>
                                    </div>
                                </div>
                                <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed mb-4">
                                    We are currently building your Docker image and pushing it to the registry.
                                </p>
